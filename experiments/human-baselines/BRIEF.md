...
anything afterwards. Today `build_prompt_table` has a `DatasetSpec` only for measles and
a stub for WHO GHO, so the four other series need spec entries before their packs can be
generated. That is backend work and is not done in this kit. `experiments/analysis/
wordcount.py` is likewise a dependency of this brief and does not exist yet.
