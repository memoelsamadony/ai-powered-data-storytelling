...
test("a dataset the backend serves replaces its snapshot", () => {
  const merged = mergeDatasets([make("measles", { rows: 9959 })], [make("measles", { rows: 12 })]);
  assert.equal(merged.length, 1);
  assert.equal(merged[0].rows, 9959);
});

test("a dataset the backend does not serve keeps its snapshot", () => {
...
test("an empty backend list falls back to every snapshot entry, in order", () => {
  const snapshot = [make("measles"), make("who-health")];
  assert.deepEqual(mergeDatasets([], snapshot), snapshot);
});

test("snapshot order is preserved, whatever order the backend answers in", () => {
...
test("merging is whole-dataset, never field-by-field", () => {
  ...
  // figures must render no map, rather than inheriting the snapshot's and
  // presenting a stale country table under a fresh trend line, where the
  // source note below could no longer say which of the two is on screen.
  const live = make("measles", { rows: 9959 });
  const mock = make("measles", {
    rows: 12,
    countryStats: [{ iso3: "NGA", name: "Nigeria", series: {} }],
    countryYears: [1990],
    countrySourceNote: "snapshot country table",
  });
