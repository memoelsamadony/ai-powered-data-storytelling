...
class TwoAxisJudgeTests(SimpleTestCase):
    ...
    def test_no_prompt_tells_the_judge_which_story_was_moderated(self):
        # The four numbers now come from two blind calls rather than one paired
        # call. A prompt that labels the treatment cannot measure it: the judge
        # would be reporting its expectation that the moderated one reads
        # calmer. Assert on every prompt string the module exposes, so
        # reintroducing a labelled prompt fails here rather than in the data.
        prompts = [v for k, v in vars(judge).items()
                   if k.isupper() and isinstance(v, str)]
        self.assertTrue(prompts)
        for text in prompts:
            for leak in ("unmoderated", "after tone moderation", "VERSION A", "VERSION B"):
...
class BlindJudgeRunTests(TestCase):
    """judge_run persists two independent scores, never a paired verdict."""

    def test_judge_run_scores_each_story_in_its_own_call(self):
        seen = []

        def fake(table, title, paragraphs, model=judge.DEFAULT_MODEL):
            seen.append(list(paragraphs))
            return judge.StoryScore(alarmism=3.0, optimism=2.0, rationale="",
                                    cost_usd=0.01, duration_s=0.1)

        run = Run.objects.create(
            dataset_id="measles", tier="demo",
            raw_title="a", raw_paragraphs=["raw body"],
            moderated_title="b", moderated_paragraphs=["moderated body"],
        )
        with mock.patch.object(judge, "score_story", side_effect=fake):
            judge.judge_run(run, "table")
        # Two calls, and neither one saw the other story's text.
